...
class Sniffer {
  constructor(logger, userDataManager, globalSettings) {
    // Wrap logger with [Sniffer] prefix
    this.logger = {
      info: (msg) => logger.info(`[Sniffer] ${msg}`),
      error: (msg) => logger.error(`[Sniffer] ${msg}`),
      warn: (msg) => logger.warn(`[Sniffer] ${msg}`),
      debug: (msg) => logger.debug(`[Sniffer] ${msg}`),
    };
  ...
  async processEthPacket(frameBuffer) {
    if (this.isPaused) return; // Don't process packets if paused
              ...
              do {
                ...
                try {
                  if (this.current_server !== src_server) {
                    ...
                    if (
                      this.globalSettings.autoClearOnServerChange &&
