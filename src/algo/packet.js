...
class PacketProcessor {
  ...
  _decompressPayload(buffer) {
    if (!zlib.zstdDecompressSync) {
      this.logger.warn(
        "zstdDecompressSync is not available! Please check your Node.js version!",
      );
      return;
    }
  ...
  _processAoiSyncDelta(aoiSyncDelta, isPaused = false) {
    ...
    for (const syncDamageInfo of skillEffect.Damages) {
      ...
      let attackerUuid =
        syncDamageInfo.TopSummonerId || syncDamageInfo.AttackerUuid;
      ...
      const isCrit =
        syncDamageInfo.TypeFlag != null
          ? (syncDamageInfo.TypeFlag & 1) === 1
          : false;
      const isCauseLucky =
        syncDamageInfo.TypeFlag != null
          ? (syncDamageInfo.TypeFlag & 0b100) === 0b100
          : false;

      const isMiss =
        syncDamageInfo.IsMiss != null ? syncDamageInfo.IsMiss : false;
      const isHeal = syncDamageInfo.Type === pb.EDamageType.Heal;
      const isDead =
        syncDamageInfo.IsDead != null ? syncDamageInfo.IsDead : false;
      const isLucky = !!luckyValue;
      const hpLessenValue =
        syncDamageInfo.HpLessenValue != null
      ...
      if (isTargetPlayer) {
        ...
        if (isHeal) {
          ...
          // Player takes damage
          this.userDataManager.addTakenDamage(
            targetUuid.toNumber(),
            damage.toNumber(),
            isDead,
          );
        ...
        // Non-player target
        if (isHeal) {
          // Non-player is healed
        } else {
          // Non-player takes damage
          if (isAttackerPlayer) {
            // Only record player-inflicted damage
      ...
      if (isAttackerPlayer) {
        ...
        if (this.userDataManager.enemyCache.name.has(attackerUuid.toNumber())) {
          infoStr += this.userDataManager.enemyCache.name.get(
            attackerUuid.toNumber(),
          );
        }
      ...
      if (isTargetPlayer) {
        ...
        if (this.userDataManager.enemyCache.name.has(targetUuid.toNumber())) {
          targetName += this.userDataManager.enemyCache.name.get(
            targetUuid.toNumber(),
          );
        }
  ...
  _processSyncToMeDeltaInfo(payloadBuffer, isPaused = false) {
    ...
    if (uuid && !currentUserUuid.eq(uuid)) {
      ...
      this.logger.info(
        "Got player UUID! UUID: " + currentUserUuid + " UID: " + localPlayerUid,
      );
      // Set the local player in UserDataManager
      this.userDataManager.setLocalPlayer(localPlayerUid);
    }
  ...
  _processSyncContainerData(payloadBuffer) {
    ...
    try {
      ...
      if (charBase.Name) {
        this.logger.debug(
          `_processSyncContainerData: Setting player name for UID ${playerUid}: ${charBase.Name}`,
        );
      ...
      if (professionList.CurProfessionId) {
        const professionName = getProfessionNameFromId(
          professionList.CurProfessionId,
        );
        this.logger.debug(
          `_processSyncContainerData: Setting profession for UID ${playerUid}: ${professionName}`,
        );
  ...
  _processSyncContainerDirtyData(payloadBuffer) {
    ...
    const syncContainerDirtyData =
      pb.SyncContainerDirtyData.decode(payloadBuffer);
    ...
    const messageReader = new BinaryReader(
      Buffer.from(syncContainerDirtyData.VData.Buffer),
    );
      ...
      case 2: // CharBase
        ...
        switch (fieldIndex) {
          ...
          case 35: // FightPoint
            ...
            messageReader.readInt32();
            this.userDataManager.setFightPoint(
              currentUserUuid.shiftRight(16).toNumber(),
              fightPoint,
            );
            break;
    ...
    for (const attr of attrs) {
      ...
      switch (attr.Id) {
        case AttrType.AttrName:
          ...
          this.logger.debug(
            `_processPlayerAttrs: Setting player name for UID ${playerUid}: ${playerName}`,
          );
        ...
        case AttrType.AttrProfessionId:
          ...
          this.logger.debug(
            `_processPlayerAttrs: Setting profession for UID ${playerUid}: ${professionName}`,
          );
          this.userDataManager.setProfession(playerUid, professionName);
          break;
  ...
  _processEnemyAttrs(enemyUid, attrs) {
    for (const attr of attrs) {
      ...
      const reader = pbjs.Reader.create(attr.RawData);
      this.logger.debug(
        `Found attrId ${attr.Id} for E${enemyUid} ${attr.RawData.toString("base64")}`,
      );
      switch (attr.Id) {
        case AttrType.AttrName:
          ...
          this.logger.info(
            `Found monster name ${enemyName} for id ${enemyUid}`,
          );
          break;
  ...
  processPacket(packets, isPaused = false, globalSettings = null) {
    try {
      ...
      do {
        ...
        const packetReader = new BinaryReader(
          packetsReader.readBytes(packetSize),
        );
